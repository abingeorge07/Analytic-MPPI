"""Render a video + provenance bundle for configs/exp/hopper_vanilla_mppi.py.

Mirrors capability_probe.py's save_bundle layout (video.mp4, config.json, metrics.json,
model/ + INFO.json) so this config's artifact sits next to the sibling mppi_fpl_K256_8s /
mppi_normal_K256_8s bundles under the same runs/capability/hopper/ directory, but is driven
straight from the config.py file (not capability_probe's ENVS/CLI sweep machinery) since
config.resolve() is the source of truth for this run.

Run:  .venv/bin/python verification/render_hopper_vanilla_video.py
"""
...
CONFIG_PATH = "configs/exp/hopper_vanilla_mppi.py"
TAG = "mppi_normal_K256_10s_weighttuned"
OUT_DIR = REPO / "runs" / "capability" / "hopper" / TAG
N_SEEDS = 30
FALL_UPRIGHT = 0.6
...
def main() -> None:
    ...
    # --- video (seed=0, hopper's tracking camera) -----------------------------------
    render_video(r.task_name, r.controller, steps=r.steps, out_path=OUT_DIR / "video.mp4",
                seed=0, cost_mode=cost_mode, init_fn=r.init_fn, camera="track",
                task_kwargs=r.task_kwargs, **build)
